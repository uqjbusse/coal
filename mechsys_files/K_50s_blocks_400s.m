function K_50s_blocks_400

% twohundredfiftysix 50s blocks forming 400s (x4)
...
%                                                          filename = ['cutTDM100_' sprintf('%d%d%d_%d%d%d_%d%d%d_%d%d%d',a,b,c,d,e,f,g,h,i,j,k,l) '_000'];                                          
                                                                    ...
                                                                    if blocks(n).SampleName(11:17) == filename(11:17) & blocks(n).sizeshort == 50

                                                                    C(n).Mother = filename
                                                                    C(n).SampleName= blocks(n).SampleName
                                                                    C(n).K_filled_mD= blocks(n).K_filled_mD  
...
[mother, ~, subs] = unique({C.Mother}); %identify identical mothers
NrOfDaughters = length(subs)/length(mother);

AllValues = cat(3,C.K_filled_mD) % a 3x3x(N*8) array
...
for n=1:(length(mother))
    family(n).sum_50s_Daughters_400 = SumValue(:,:,1,n)
end
...
sum_value_check = (C(769).K_filled_mD)
for k=770:1024
    value_add= (C(k).K_filled_mD)
